...
def pageRankGenerator2(Ato = [array((), int64)],
                       Acc = [array((), int64)],
                       numLinksTo = array((), int64),
                       numLinksCc = array((), int64),
                       ln = array((), int64),
                       alpha = 0.85,
                       convergence = 0.0001,
                       checkSteps = 10
    ...
    numLinks = add(numLinksTo, numLinksCc)
    ...
    numLinksTo = numLinksTo.astype(float64)
    numLinksCc = numLinksCc.astype(float64)
    for i in range(N):
        if numLinksTo[i] and numLinksCc[i]:
            numLinksTo[i] = 0.9 / numLinksTo[i]
            numLinksCc[i] = 0.1 / numLinksCc[i]
        elif numLinksTo[i]:
            numLinksTo[i] = 1.0 / numLinksTo[i]
            numLinksCc[i] = 0.0
        elif numLinksCc[i]:
            numLinksTo[i] = 0.0
            numLinksCc[i] = 1.0 / numLinksCc[i]
        else:
            numLinksTo[i] = 0.0
            numLinksCc[i] = 0.0
    ...
    while not done:
        ...
        for step in range(checkSteps):
            ...
            while ii < N:
                pageTo = Ato[ii]
                pageCc = Acc[ii]
#                if pageTo.shape[0] and pageCc.shape[0]:
#                    page = concatenate((pageTo, pageCc))
#                elif pageTo.shape[0]:
#                    page = pageTo
#                else:
#                    page = pageCc
#                hOrig = 0
#                if page.shape[0]:
#                    hOrig = alpha * dot(iOld.take(page, axis = 0), 1. / numLinks.take(page, axis = 0))
                    
                h = 0
                if pageTo.shape[0] and pageCc.shape[0]:
                    h += alpha * dot(iOld.take(pageTo, axis = 0), numLinksTo.take(pageTo, axis = 0)) 
                    h += alpha * dot(iOld.take(pageCc, axis = 0), numLinksCc.take(pageCc, axis = 0))
                elif pageTo.shape[0]:
                    h += alpha * dot(iOld.take(pageTo, axis = 0), numLinksTo.take(pageTo, axis = 0))
                elif pageCc.shape[0]:
                    h += alpha * dot(iOld.take(pageCc, axis = 0), numLinksCc.take(pageCc, axis = 0))
...
def pageRank2(
    ...
    @see pageRankGenerator for parameter description
    """
    incomingLinksTo, numLinksTo, leafNodesTo = transposeLinkMatrix(linkToMatrix)
    incomingLinksCc, numLinksCc, leafNodesCc = transposeLinkMatrix(linkCcMatrix)

    numLinks = add(numLinksTo,numLinksCc)
    leafNodes= intersect1d(leafNodesTo,leafNodesCc)
    
    for gr in pageRankGenerator2(incomingLinksTo, incomingLinksCc,
                                 numLinksTo, numLinksCc,
                                 leafNodes,
                                 alpha = alpha,
